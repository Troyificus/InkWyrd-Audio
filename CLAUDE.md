...
**It now disables the RPC auto-mute client too** (sprite-skin branch).
Before, it only stopped the bot: a launch test reading the user's real
settings would still start `DiscordRpcClient` - able to mute/unmute
their actual Discord client and spend their RPC refresh token. It does
NOT stop audio: a test launch plays through the speakers if the user's
saved Monitor setting is on.
...
### Sprite skins: per-widget images (branch `sprite-skin`, not yet released)

The user asked for a sprite-based, Winamp-STYLE skin (their words: plan
"a sprite-based LookAndFeel for a classic Winamp-style skin"). That
supersedes the beta.16 note above that Winamp was "only an example" -
but the constraints they set keep it far from a Winamp clone:

- **Every Player control stays on the Player's main face** (positions may
  move, nothing moves to another window).
- **Resizable nine-slice everywhere** - no fixed-size bitmap faces.
- **Original art only.** Never Nullsoft's bitmaps, no `.wsz` loading.
- Their local **ComfyUI** as a starting point for the art.

How it's built:

- **An optional `"sprites"` section of the existing `skin.json`**, not a
  separate skin system: sheet + `items` of `{rect, slice, tile}`. Parsed
  by `SkinSprites` (`src/app/gui/SkinSprites.{h,cpp}`), pure over
  `juce::var` + `juce::Image`, compiled into the test target. Schema
  stays 1: an older build ignores the unknown key and shows the palette.
- **Every LookAndFeel method asks for a sprite first and falls back to
  the drawn code**, so a skin can supply one sprite and the built-in look
  (no sprites) is untouched. Covered: window body
  (`fillResizableWindowBackground`), title bar (+`@inactive`), title
  buttons, buttons, icons, tick boxes, sliders (track/fill/thumb, fill is
  CLIPPED not squeezed), scrollbars, text fields, combos, popups,
  `drawPanel`/`drawInsetWell` (the Player display asks for `display`,
  falling back to `well`).
- **States fall back through chains** (`@ondown` -> `@on` -> `@down` ->
  plain). **Per-control sprites** are `<base>.<componentId>` and win over
  the generic one through the whole chain. `icon.<id>` replaces a
  button's LABEL, drawn at natural size - that's how glyph buttons stay
  resizable (background nine-slices, glyph doesn't). The label text stays
  set, so UI Automation still finds buttons by name.
- **`SkinSpriteNames.h` is the single list** of sprite names and
  component IDs. The C++ typo check, the self-test, and
  `tools/skin-builder` all read it. `build_skin.py --check` greps every
  `setComponentID` against it.
- **The Player's on/off buttons now carry real toggle state**
  (`setToggleState(..., dontSendNotification)` alongside the existing
  text) - a sprite needs it to light up. Visible in the DRAWN look too:
  engaged toggles now fill with `accentSoft`, and **Mic muted lights in
  `warning`** (the button's own `buttonOnColourId`), not green.
- Pixel art: `lowResamplingQuality`, which JUCE 8's Direct2D renderer
  maps to `D2D1_INTERPOLATION_MODE_NEAREST_NEIGHBOR` (checked in
  `juce_Direct2DGraphicsContext_windows.cpp`, not assumed). `scale` is
  screen px per sheet px; optional `sheet2x` is used once a sheet pixel
  covers >= 1.5 physical pixels.
- **Pixel Phosphor**, the example sprite skin, is generated by
  `tools/skin-builder/build_skin.py`, committed under
  `src/app/skins/Pixel Phosphor/`, embedded with `juce_add_binary_data`
  and written once on first run behind its own flag
  (`spriteExampleSkinWritten`, separate from the palette examples' flag
  so existing users get it). It never overwrites a folder of that name.
- `comfy_textures.py` drives ComfyUI (Flux dev fp8) for the window and
  display textures only; `build_skin.py --textures` crops, flattens the
  lighting, pixelises, tiles and posterises them. **The shipped example
  uses no generated textures** - model licence not yet checked. See
  `tools/skin-builder/README.md` for what made textures tile.

Verification: 40-odd sprite checks in `INKWYRD_SELFTEST` (fallback
chains, every author mistake, and actual pixels: corners keep their
size, the middle stretches without blending, scale 2 doubles, tiles
repeat). The drawing checks were **seen to fail** against deliberately
broken nine-slice/tiling code before being trusted. **`INKWYRD_SKINRENDER=<dir>`
(+ `INKWYRD_SKIN=<skin folder>`)** paints a gallery of real widgets through
the real LookAndFeel at 1x and 2x - use it to judge art without launching
the app. One real launch confirmed Player/Library/Playlist all skinned.

**Gotcha that made every pixel check fail at first:** on Windows a
`juce::Image` is a Direct2D bitmap by default, and nothing drawn reaches
its pixels until the `juce::Graphics` is DESTROYED. Scope the Graphics
before `getPixelAt()`.

**Gotcha in the build:** editing a header while MSBuild is mid-build can
leave an .obj compiled against the OLD header but timestamped after the
edit - it isn't rebuilt, and a struct layout mismatch between two .objs
shows up as heap corruption in a destructor (here: `~SkinLoadResult`).
Touch the edited files and rebuild.

Not done: a pixel/bitmap FONT (fonts stay installed-family names - a
bundled font is a licensing decision), sprite soundboard pads (pads carry
the user's own colours), table headers, and deriving layout row heights
...
**THE REDIRECTION DOES NOT PROTECT THE USER'S FILES. Do not treat it as a
sandbox.** Learned the hard way in the sprite-skin session (2026-09-23):
a probe file CREATED in `%APPDATA%` from a session did land in the
package copy, which made launch-testing look safe - but
`%APPDATA%\Inkwyrd Audio\` already existed for real, and **every write
into it went to the REAL folder**: the session's own edit of
`Inkwyrd Audio.settings`, the test app's saves, its `log.txt`, a new
skins folder. The package copy of `Inkwyrd Audio` never existed at all.
The user's real `skinName` was overwritten (its old value is unknown - no
backup had been taken) and their previous `log.txt` was lost. The
hash-before/after check in the launch script is what caught it.

So the standing rule above is the ONLY protection: **back up the real
`%APPDATA%\Inkwyrd Audio\` folder (via WMI, to see the real one) before
any launch or any edit of its files, and restore it afterwards.** A test
launch also reads the user's real settings: use `INKWYRD_NO_DISCORD=1`,
and expect it to play audio if their Monitor setting is on.
